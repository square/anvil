...
internal fun ClassId.generatedAnvilSubcomponent(parentClass: ClassId): ClassId {
  ...
  val parentPackageName = parentClass.packageFqName
    .safePackageString(dotPrefix = false, dotSuffix = false)

  val segments = buildList {
    val parentPackage = if (parentPackageName.startsWith(HINT_PACKAGE)) {
      // This happens if the parent is generated by Anvil itself. Avoid nesting too deeply.
      // short "n" for "nested"
      parentPackageName.removePrefix(HINT_PACKAGE)
        .removePrefix(".")
    ...
    addAll(parentClassPackageSuffix.split('.'))
    addAll(relativeClassName.pathSegments().map { it.asString() })
  }

  val relativeClassName = segments
    .joinToString(separator = "_", postfix = ANVIL_SUBCOMPONENT_SUFFIX)

  return ClassId(FqName(HINT_PACKAGE), FqName(relativeClassName), false)
}
